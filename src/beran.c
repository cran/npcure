...
// inline function called by beran
inline static void berancore(double *pdatax,
			     double *pdatat,
			     int *pdatadelta,
			     int nrow,
			     double x,
			     ...
			     double *presult) {
  int i, lasti, j;
  double xhi, Bx[nrow], sumBx = 0.0, temp = 1.0, pberan[nrow];
  bool found;
  for (i = 0; i < nrow; i++) {
...
SEXP berannp0(SEXP Datat,
	      ...
	      SEXP T,
	      SEXP Lt) {
  int conth, i, nrow = asInteger(Nrow), lx = asInteger(Lx), lh = asInteger(Lh), local = asLogical(Local), *pdatadelta, lt = asInteger(Lt), nroworlt;
  double x, h, *px, *pdatax, *ph, *presult, *pdatat, *ppt = NULL;
  bool testim = false;
  px = REAL(X);
  pdatax = REAL(Datax);
  ...
  if (local) {
    ...
    for (i = 0; i < lx; i++) {
      ...
      h = ph[i];
      x = px[i];
      berancore(pdatax, pdatat, pdatadelta, nrow, x, h, lt, testim, ppt, presult);
...
// Function for computing confidence bands for Beran's survival estimator
SEXP berannp0confband(SEXP Datat,
		      SEXP Datax,
		      SEXP Datadelta,
		      SEXP Nrow,
		      SEXP X,
  ...
  if (LOGICAL(Local)[0]) {
    ...
    for (contx = 0; contx < lx; contx++) { // 1. for (in contx)
      ...
      pup = REAL(up);
      SEXP beranboot = PROTECT(allocVector(REALSXP, nrow));
      REAL(x0)[0] = px[contx];
      REAL(h0)[0] = ph[contx];
	...
	beranboot = berannp0(tbootorder, xbootorder, deltabootorder, Nrow, x0, lx0, h0, lx0, localdummy, Datat, Nrow);
	pberanboot = REAL(VECTOR_ELT(beranboot, 0));
	for (i = 0; i < nrow; i++) {
	  mterm = pberanboot[i];
	  m[i] += mterm;
	  m2[i] += mterm*mterm;
